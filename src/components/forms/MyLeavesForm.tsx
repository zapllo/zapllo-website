...
import { Paperclip, Mic, Calendar, Square } from "lucide-react";
...
const MyLeaveForm: React.FC<LeaveFormProps> = ({ leaveTypes, onClose }) => {
  ...
  // Helper function to format time as MM:SS (e.g., 00:01)
        ...
        <Dialog.Content className="fixed z-[50] inset-0 flex items-center justify-center">
          <div className="bg-[#1A1C20] overflow-y-scroll scrollbar-hide h-fit max-h-[600px]  shadow-lg w-full   max-w-md p-6 rounded-lg">
            <div className="flex justify-between mb-4">
              <Dialog.Title className="text-md mb-4 font-medium">
                Submit Leave Request
              </Dialog.Title>
              <Dialog.DialogClose className="-mt-4">
                <img
                  src="/icons/cross.png"
                  className="h-5 hover:bg-[#121212] rounded-full"
                />
              </Dialog.DialogClose>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4 ">
              <div className="relative">
                <label className="absolute bg-[#1A1C20] ml-2 text-xs -mt-2 px-1">
                  Leave Type
                </label>
                <select
                  name="leaveType"
              ...
              <div className="flex items-center bg-[#282D32] px-3 rounded-lg gap-3">
                <canvas
                  ref={canvasRef}
                  className={` ${
                    recording ? `w-[400px] h-20 rounded-lg` : "hidden"
                  } `}
                ></canvas>
                {recording && (
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div
                      className=" flex items-center gap-1 cursor-pointer bg-gray-700 p-2 rounded-md text-xs "
