                ...
                <div className="border overflow-y-scroll scrollbar-hide h-10/11 p-4 rounded-lg">
                                ...
                                <div className="flex gap-2 justify-start">
                                    <div className="h-6 w-6 rounded-full bg-primary">
                                        <h1 className="text-center uppercase text-xs mt-1">
                                            {selectedLeave.user.firstName[0]}{selectedLeave.user.lastName[0]}
                                        </h1>
                                    </div>
                                    <h1 id="userName" className="col-span-3 text-sm">{`${selectedLeave.user.firstName} ${selectedLeave.user.lastName}`}</h1>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                {selectedLeave.approvedBy.firstName && (
                                    <div className="flex gap-2 justify-start">
                                        ...
                                        <h1 id="approvedBy" className="col-span-3 text-sm">{`${selectedLeave.approvedBy.firstName} ${selectedLeave.approvedBy.lastName}`}</h1>
                                    </div>
                                )}
                            </div>
                        )}
                    ...
                    <div className='flex gap-28 ml-1'>
                        <div className="flex items-center gap-1 mt-4">
                            ...
                            <Label htmlFor="fromDate" className="text-right text-sm">From Date</Label>
                            <div className="flex gap-2 ml-2 text-xs justify-start">
                                <h1 id="fromDate" className="col-span-3">{new Date(selectedLeave.fromDate).toLocaleDateString()}</h1>
                            </div>
                        </div>

                        <div className="flex items-center gap-1 mt-4">
                            <Clock className="h-4 text-[#E94C4C]" />
                            <Label htmlFor="toDate" className="text-right text-sm">To Date</Label>
                            <div className="flex gap-2 ml-5 justify-start">
                                <h1 id="toDate" className="col-span-3 text-xs">{new Date(selectedLeave.toDate).toLocaleDateString()}</h1>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Status */}
                    <div className="flex items-center gap-1 mt-4">
                        {selectedLeave.status === 'Pending' && <Circle className="h-4 text-red-500" />}
                        {selectedLeave.status === 'Approved' && <CheckCircle className="h-4 text-green-500" />}
                        <Label htmlFor="status" className="text-right text-sm">Status</Label>
                        <div className="flex gap-2 ml-12 justify-start">
                            <h1 id="status" className="col-span-3 text-xs ml-1">{selectedLeave.status}</h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 mt-4">
                        ...
                        <Label htmlFor="leaveType" className="text-right text-sm">Leave Type</Label>
                        <div className="flex ml-6 justify-start">
                            <h1 id="leaveType" className="col-span-3 text-xs">{selectedLeave.leaveType.leaveType}</h1>
                        </div>
                    </div>
                    <div className="flex items-center gap-1 mt-4">
                        <Calendar className="h-4 text-gray-400" />
                        <Label htmlFor="leaveType" className="text-right text-sm">Applied For</Label>
                        <div className="flex ml-5 justify-start">
                            <h1 id="leaveType" className="col-span-3 text-xs"> {selectedLeave.appliedDays} Day(s)</h1>
                        </div>
                    </div>
                    <div className="flex items-center gap-1 mt-4">
                        <CalendarCheck className="h-4 text-green-500" />
                        <Label htmlFor="leaveType" className="text-right text-sm">Approved For</Label>
                        <div className="flex ml-2 justify-start">
                            <h1 id="leaveType" className="col-span-3 text-xs">
                                {selectedLeave.leaveDays.filter((day) => day.status === 'Approved').length} Day(s)
                            </h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 mt-4">
                        ...
                        <Label htmlFor="leaveType" className="text-right text-sm">Reason</Label>
                        <div className="flex ml-12 justify-start">
                            <h1 id="leaveType" className="col-span-3 text-xs">{selectedLeave.leaveReason}</h1>
                        </div>
                    </div>
