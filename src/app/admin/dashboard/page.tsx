...
import { Bell, Home, LogOut, MessageCircleReply, Users, Eye } from 'lucide-react';
...
const DashboardPage = () => {
  ...
  const getTaskUsersCount = (org: Organization) => {
    ...
    const taskUsers = tasks
      .filter(task => task.assignedUser && task.assignedUser.organization === org._id)
    ...
    <div className="flex h-screen bg-[#201124]">
      ...
      <div className="flex-1 p-6">
        ...
        <div className="flex gap-4 mb-6">
          {['Today', 'Yesterday', 'This Week', 'Last Week', 'This Month', 'Last Month', 'This Year', 'All Time', 'Custom'].map((tab) => (
            <button key={tab} className="px-4 py-2 bg-[#1f1b4e] text-white rounded-full">{tab}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-6 mb-6">
          <div className="p-6 bg-transparent border rounded-lg text-center">
            <div className="text-white mt-2">Total Task</div>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="flex justify-between items-center mb-4">
            <input
              className="px-4 py-2 rounded-full bg-[#3c366b] text-white"
              placeholder="Search..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <div className="flex items-center gap-4">
              <button className="bg-[#3c366b] p-2 rounded-full text-white">Filter</button>
              <button className="bg-[#3c366b] p-2 rounded-full text-white">Download</button>
            </div>
          </div>
          ...
          <table className="w-full text-white">
            <thead>
              <tr className="text-left">
                ...
                <th className="pb-2">Subscribed Users</th>
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <button
              className="text-white"
              onClick={() => setCurrentPage(prev => Math.min(prev + 1, Math.ceil(filteredOrganizations.length / itemsPerPage)))}
